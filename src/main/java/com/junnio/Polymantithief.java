    ...
    public void onInitialize() {
        ...
        ServerPlayNetworking.registerGlobalReceiver(ShulkerLogPayload.ID, (payload, context) -> {
            context.server().execute(() -> {
                DatabaseManager.insertLog(payload.playerName(), payload.actionName(), payload.itemName(), payload.shulkerName(), payload.x(), payload.y(), payload.z(), payload.dimension(), payload.isContainer());
            });
        });
        ...
        UseBlockCallback.EVENT.register((player, world, hand, hitResult) -> {
            ...
            if (beAbove instanceof ShulkerBoxBlockEntity shulker && shulker.hasCustomName()) {
                if (shulker.hasCustomName()) {
                    ...
                    DatabaseManager.insertLog(playerName, "placed-hopper", "", shulkerName, x, y, z, dimension, false);
                }
            }
            return ActionResult.PASS;
        });
